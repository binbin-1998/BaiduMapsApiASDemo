 ...
 * 同时展示如何进行节点浏览并弹出泡泡
 */
public class MassTransitRoute extends AppCompatActivity implements BaiduMap.OnMapClickListener,
    ...
    public void onGetMassTransitRouteResult(MassTransitRouteResult result) {
        if (result != null && result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
            // 起终点模糊，获取建议列表
            Toast.makeText(MassTransitRoute.this, "起终点或途经点地址有岐义，通过result.getSuggestAddrInfo()接口获取建议查询信息", Toast.LENGTH_SHORT).show();
            return;
        }

        if (result == null || result.error == SearchResult.ERRORNO.RESULT_NOT_FOUND) {
            Toast.makeText(MassTransitRoute.this, "抱歉，未找到结果", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (result.error == SearchResult.ERRORNO.NO_ERROR) {
            ...
            if (!hasShowDialog) {
                // 列表选择
                SelectRouteDialog selectRouteDialog = new SelectRouteDialog(MassTransitRoute.this,
