 ...
 * 同时展示如何进行节点浏览并弹出泡泡
 */
public class TransitRoutePlan extends AppCompatActivity implements BaiduMap.OnMapClickListener,
    ...
    public void onGetTransitRouteResult(TransitRouteResult result) {
        if (result != null && result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
            ...
            Toast.makeText(TransitRoutePlan.this, "起终点或途经点地址有岐义，通过result.getSuggestAddrInfo()接口获取建议查询信息",
                    Toast.LENGTH_SHORT).show();
            return;
        }

        if (result == null || result.error == SearchResult.ERRORNO.RESULT_NOT_FOUND) {
            Toast.makeText(TransitRoutePlan.this, "抱歉，未找到结果", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (result.error == SearchResult.ERRORNO.NO_ERROR) {
            ...
            if (result.getRouteLines().size() > 1) {
                ...
                if (!hasShownDialogue) {
                    SelectRouteDialog selectRouteDialog = new SelectRouteDialog(TransitRoutePlan.this,
